...
// like check_associator but focused on visual subfield. if there are 
//    changes to that subfield then an output file is written.
// image output is restricted to subfield
#define LEFT_BOUND_DEG     180.0
#define RIGHT_BOUND_DEG    250.0

#define LEFT_BOUND32    ((uint32_t) (LEFT_BOUND_DEG * DEG_TO_BAM32))
#define RIGHT_BOUND32   ((uint32_t) (RIGHT_BOUND_DEG * DEG_TO_BAM32))

// assume pan images are 10 pix/deg
#define IMAGE_WIDTH     3600
#define LEFT_BOUND_PIX  ((uint32_t) (LEFT_BOUND_DEG * 10.0))
#define RIGHT_BOUND_PIX ((uint32_t) (RIGHT_BOUND_DEG * 10.0))
      ...
      )
{
   ...
   assert(chan >= 0);
   assert(chan <= 2);
   if (((target->left > LEFT_BOUND_DEG) && 
         (target->left < RIGHT_BOUND_DEG)) ||
         ((target->right > LEFT_BOUND_DEG) && 
         (target->right < RIGHT_BOUND_DEG))) {
      pan_state_[chan] = 1;
...
printf("  Loading %s\n", fname);
      ...
      // code presently hardwired for 10ppd
      assert(pan_in_->size.width == IMAGE_WIDTH);
   }
   /////////////////////////////
      ...
      )
{
   FILE *fp = NULL;
   int rc = -1;
   const uint32_t cols = RIGHT_BOUND_PIX - LEFT_BOUND_PIX;
   rgb_pix_type rgb_row[cols];
   printf("Writing %s\n", filename);
   ...
   {
      uint32_t idx = r * IMAGE_WIDTH + LEFT_BOUND_PIX;
...
printf("Saving %s\n", fname);
         write_subimage(fname, pan_out_[i]->size, pan_out_[i]->rgb);
      }
   }
}
